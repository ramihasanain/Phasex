...
function getSymbolName(code: string): { ar: string; en: string } {
  ...
  if (code.length === 6 && /^[A-Z]{6}$/.test(code)) {
    return {
      ar: `${code.slice(0, 3)}/${code.slice(3)}`,
      en: `${code.slice(0, 3)}/${code.slice(3)}`,
    };
  }
  ...
  useEffect(() => {
    ...
    async function fetchMarkets() {
      try {
        ...
        const apiMarkets: MarketInfo[] = json || [];
        ...
        if (marketsWithAll.length > 0) {
          const defaultMarket =
            marketsWithAll.find((m) => m.code === "FOREX") || marketsWithAll[0];
          setSelectedMarket(defaultMarket);
        }
      } catch (err: any) {
        if (!cancelled)
          setMarketsError(err.message || "Failed to fetch markets");
      } finally {
