...
export function TradingDashboard({ onLogout, onOpenDynamics }: TradingDashboardProps) {
  ...
  const [quickTradeModal, setQuickTradeModal] = useState<{ symbol: string; action: string } | null>(null);
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" dir={isRTL ? "rtl" : "ltr"} style={{ background: tk.bg, fontFamily: "'Inter', system-ui, sans-serif", transition: "background 0.3s" }}>
      ...
      <header className="relative z-40" style={{
        ...
        {tk.isDark && <motion.div className="absolute top-0 left-0 right-0 h-[1.5px] z-10"
        ...
        <div className="flex items-center justify-between px-5 py-2.5 relative z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.button onClick={() => setIsProfileOpen(true)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold cursor-pointer"
              style={{ color: tk.textMuted, background: tk.buttonGhost, border: `1px solid ${tk.buttonGhostBorder}`, backdropFilter: tk.isDark ? 'blur(8px)' : undefined }}>
            ...
            <div className="relative" ref={themeDropdownRef}>
              <motion.button onClick={() => setThemeDropdownOpen(!themeDropdownOpen)} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.96 }}
                className="w-8 h-8 rounded-xl flex items-center justify-center cursor-pointer text-[16px]"
                style={{ background: tk.buttonGhost, border: `1px solid ${tk.buttonGhostBorder}` }}>
              ...
              <AnimatePresence>
                {themeDropdownOpen && (
                  <motion.div
                    ...
                    style={{ background: tk.isDark ? 'rgba(6,10,16,0.95)' : tk.surfaceElevated, border: `1px solid ${tk.isDark ? 'rgba(99,102,241,0.12)' : tk.border}`, backdropFilter: tk.isDark ? 'blur(20px)' : undefined, right: isRTL ? 'auto' : 0, left: isRTL ? 0 : 'auto' }}
                  >
                    <div className="py-1 flex flex-col">
                      {themeOptions.map((opt) => (
                        ...
                        >
                          ...
                          <span className={theme === opt.key ? "font-bold" : ""}>
            ...
            <motion.button onClick={() => setIsSubscriptionOpen(true)} whileHover={{ scale: 1.04, boxShadow: '0 6px 25px rgba(250,204,21,0.15)' }} whileTap={{ scale: 0.96 }}
              ...
              style={{ color: tk.warning, background: tk.warningBg, border: `1px solid ${tk.isDark ? 'rgba(250,204,21,0.15)' : 'rgba(217,119,6,0.2)'}`, boxShadow: tk.isDark ? '0 0 20px rgba(250,204,21,0.03)' : 'none' }}>
              {/* Shimmer on subscription button */}
              <motion.div className="absolute inset-0 pointer-events-none"
                style={{ background: `linear-gradient(90deg, transparent, ${tk.isDark ? 'rgba(250,204,21,0.06)' : 'rgba(217,119,6,0.06)'}, transparent)` }}
                animate={{ x: ['-100%', '200%'] }}
                transition={{ duration: 3, repeat: Infinity, ease: 'linear' }} />
              <Crown className="w-4 h-4 relative z-10" />
              <span className="tracking-wide relative z-10">{t("subscription")}</span>
              <span className="text-[10px] font-black px-2 py-0.5 rounded-lg ml-1 relative z-10" style={{ background: tk.warningBg, color: tk.warning, border: `1px solid ${tk.isDark ? 'rgba(250,204,21,0.15)' : 'rgba(217,119,6,0.15)'}` }}>
      ...
      <div className="flex gap-0 py-3" style={{ minHeight: "calc(100vh - 52px)" }}>
        ...
        <div className="flex-1 flex flex-col gap-3 min-w-0 px-0 ml-3">
          ...
          <div className="rounded-2xl overflow-hidden mb-1 flex-shrink-0 relative"
            style={{ background: tk.isDark ? 'radial-gradient(ellipse at 50% 0%, rgba(99,102,241,0.04) 0%, rgba(6,10,16,0.95) 60%)' : tk.surface, border: `1px solid ${tk.isDark ? 'rgba(99,102,241,0.1)' : tk.border}`, backdropFilter: tk.isDark ? 'blur(16px)' : undefined, transition: "background 0.3s" }}>
            {/* Grid bg — dark only */}
            {tk.isDark && <div className="absolute inset-0 pointer-events-none z-0" style={{
              backgroundImage: 'linear-gradient(rgba(99,102,241,0.015) 1px, transparent 1px), linear-gradient(90deg, rgba(99,102,241,0.015) 1px, transparent 1px)',
            ...
            <button
              ...
              style={{ background: tk.isDark ? 'linear-gradient(90deg, rgba(6,10,16,0.9) 60%, transparent 100%)' : `linear-gradient(90deg, ${tk.surface} 60%, transparent 100%)` }}
            >
            ...
            <button
              ...
              style={{ background: tk.isDark ? 'linear-gradient(270deg, rgba(6,10,16,0.9) 60%, transparent 100%)' : `linear-gradient(270deg, ${tk.surface} 60%, transparent 100%)` }}
            >
          ...
          <div className="flex-shrink-0 relative" style={{ minHeight: "420px", height: "calc(100vh - 280px)" }}>
            ...
            <AnimatePresence mode="wait">
              <IndicatorChart
                ...
                renderTradeButtons={selectedAsset ? () => (
                  <div className="flex items-center gap-1.5 ml-2">
                    <motion.button
                      whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }}
                      onClick={() => { setQuickTradeModal({ symbol: selectedAsset.symbol, action: 'BUY' }); setQtSL(''); setQtTP(''); setQtSymbol(selectedAsset.symbol); setQtError(null); }}
                    ...
                    <motion.button
                      whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }}
                      onClick={() => { setQuickTradeModal({ symbol: selectedAsset.symbol, action: 'SELL' }); setQtSL(''); setQtTP(''); setQtSymbol(selectedAsset.symbol); setQtError(null); }}
                      className="px-3 py-1 rounded-lg text-[10px] font-black tracking-wider cursor-pointer"
      ...
      <AnimatePresence>
        {quickTradeModal && (
          <motion.div
            ...
            onClick={(e) => { if (e.target === e.currentTarget) setQuickTradeModal(null); }}>
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
              className="w-full max-w-md mx-4 rounded-2xl overflow-hidden relative"
              style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(99,102,241,0.06) 0%, rgba(6,10,16,0.98) 60%)', border: '1px solid rgba(99,102,241,0.15)', boxShadow: '0 25px 80px rgba(0,0,0,0.5)' }}>
              <div className="relative z-10 p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: quickTradeModal.action.includes('BUY') ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)' }}>
                      {quickTradeModal.action.includes('BUY') ? <TrendingUp className="w-4 h-4" style={{ color: '#34d399' }} /> : <TrendingDown className="w-4 h-4" style={{ color: '#f87171' }} />}
                    </div>
                    <div>
                      <h3 className="text-[14px] font-black text-white">{quickTradeModal.action} {quickTradeModal.symbol}</h3>
                      <p className="text-[10px] text-gray-500">Execute Order</p>
                    </div>
                  </div>
                  <button onClick={() => setQuickTradeModal(null)} className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white cursor-pointer"><X className="w-4 h-4" /></button>
                </div>

                <form onSubmit={async (e) => {
                  e.preventDefault();
                  setQtError(null);
                  if (!mt5Connected) { setQtError('⚠️ Connect to MT5 first'); return; }
                  setQtExecuting(true);
                  try {
                    const actionType = quickTradeModal.action.includes('BUY') ? 'BUY' : 'SELL';
                    const slVal = qtSL ? parseFloat(qtSL) : undefined;
                    const tpVal = qtTP ? parseFloat(qtTP) : undefined;
                    await executeTrade(qtSymbol, actionType, 0.01, slVal, tpVal);
                    setQuickTradeModal(null);
                  } catch (err: any) { setQtError(err.message || 'Trade failed'); }
                  finally { setQtExecuting(false); }
                }} className="space-y-3">
                  <div>
                    <label className="text-[10px] font-bold tracking-widest uppercase mb-1 block text-gray-500">Symbol Override</label>
                    <input type="text" value={qtSymbol} onChange={(e) => setQtSymbol(e.target.value)}
                      className="w-full px-3 py-2 rounded-xl text-[13px] font-bold outline-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold tracking-widest uppercase mb-1 block text-gray-500">Stop Loss</label>
                      <input type="number" step="any" value={qtSL} onChange={(e) => setQtSL(e.target.value)} placeholder="Optional"
                        className="w-full px-3 py-2 rounded-xl text-[13px] font-bold outline-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
                    <div>
                      <label className="text-[10px] font-bold tracking-widest uppercase mb-1 block text-gray-500">Take Profit</label>
                      <input type="number" step="any" value={qtTP} onChange={(e) => setQtTP(e.target.value)} placeholder="Optional"
                        className="w-full px-3 py-2 rounded-xl text-[13px] font-bold outline-none" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff' }} />
                    </div>
                  </div>
                  {qtError && <div className="text-[11px] font-bold px-3 py-2 rounded-lg" style={{ color: '#f87171', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)' }}>{qtError}</div>}
                  <motion.button type="submit" disabled={qtExecuting}
                    whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    className="w-full py-3 rounded-xl text-[12px] font-black tracking-widest uppercase cursor-pointer"
                    style={{ background: quickTradeModal.action.includes('BUY') ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)', color: quickTradeModal.action.includes('BUY') ? '#34d399' : '#f87171', border: `1px solid ${quickTradeModal.action.includes('BUY') ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
                    {qtExecuting ? 'Executing...' : `Execute ${quickTradeModal.action}`}
                  </motion.button>
                </form>
              </div>
            </motion.div>
