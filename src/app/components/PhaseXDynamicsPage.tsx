                                    ...
                                    <div className="px-14 py-8 relative z-10 flex gap-8 items-center h-full">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-gray-200 text-lg leading-relaxed font-medium" style={{ textShadow: "0 2px 4px rgba(0,0,0,0.5)" }} dir={isRTL ? "rtl" : "ltr"}>
                                                {isRTL ? (
                                                    <>
                                                        تشير البيانات الحالية إلى الاتجاه: <span style={{ color: accent }}>{tv(data.dynamics.primaryTrend)}</span> مدعوماً بـ الزخم: <span style={{ color: accent }}>{tv(data.dynamics.momentumState)}</span> و الانحياز: <span style={{ color: accent }}>{tv(data.dynamics.structuralBias)}</span>. يتشكل هذا السلوك ضمن المرحلة: <span style={{ color: accent }}>{tv(data.phase)}</span>، مع دخول السوق في حالة المرحلة السوقية: <span style={{ color: accent }}>{tv(data.dynamics.marketPhase)}</span>. يظهر مستوى التذبذب: <span style={{ color: accent }}>{tv(data.volatility)}</span>، بينما يتم تقييم مخاطر الانعكاس: على أنها <span style={{ color: accent }}>{tv(data.risk)}</span>. وتعكس درجة الثقة: <span style={{ color: accent }}>{data.confidence >= 70 ? "عالية" : data.confidence >= 40 ? "متوسطة" : "منخفضة"}</span> وضوح الإشارة الكمية وتوافق العوامل الداعمة للحالة السوقية.
                                                    </>
                                                ) : (
                                                    <>
