...
export function MarketList({
  ...
  const accentRgb = currentVisual.accent;

  /* ── Collapsed State ── */
  if (isCollapsed) {
    return (
      <div className="h-full rounded-2xl flex flex-col items-center py-3 gap-2 relative overflow-hidden"
        style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(99,102,241,0.04) 0%, rgba(6,10,16,0.95) 60%)', border: '1px solid rgba(99,102,241,0.1)', backdropFilter: 'blur(16px)' }}>
        {onToggleCollapse && (
          <button onClick={onToggleCollapse} className="w-9 h-9 rounded-xl flex items-center justify-center mb-2 cursor-pointer"
            style={{ color: '#64748b', background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.1)' }}>
  ...
  return (
    <div className="h-full rounded-2xl flex flex-col overflow-hidden relative"
      ...
      <div className="px-4 pt-4 pb-2 flex items-center justify-between relative z-10"
        ...
        <div className="flex items-center gap-2.5">
          <motion.div className="w-8 h-8 rounded-xl flex items-center justify-center"
            ...
            animate={{ boxShadow: ['0 0 0 rgba(99,102,241,0)', '0 0 15px rgba(99,102,241,0.1)', '0 0 0 rgba(99,102,241,0)'] }}
            transition={{ duration: 3, repeat: Infinity }}>
            <TrendingUp className="w-4 h-4" style={{ color: '#818cf8' }} />
          </motion.div>
          <div>
            <h2 className="text-sm font-black text-white tracking-wide">{t("markets")}</h2>
            <span className="text-[9px] font-bold tracking-wider" style={{ color: '#475569' }}>
      ...
      <div className="px-3 py-2.5 relative z-10">
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'rgba(99,102,241,0.03)', border: '1px solid rgba(99,102,241,0.06)' }}>
          {marketsLoading ? (
              ...
              return (
                <motion.button key={m.id} onClick={() => { onMarketSelect(m); setSearch(""); }}
                  ...
                  style={{
                    background: active ? `rgba(99,102,241,0.08)` : "transparent",
                    border: active ? `1px solid rgba(99,102,241,0.15)` : "1px solid transparent",
                    boxShadow: active ? '0 2px 10px rgba(99,102,241,0.08)' : 'none',
                  }}>
                  <span className="text-[18px] leading-none">{vis.emoji}</span>
                  <span className="text-[9px] font-bold tracking-wider" style={{ color: active ? '#818cf8' : '#475569' }}>
      ...
      <div className="px-4 py-1.5 relative z-10">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] font-bold tracking-wider" style={{ color: '#64748b' }}>{positivePct}% {t("positive")}</span>
          <div className="flex items-center gap-3 text-[9px]">
            <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /><span style={{ color: '#475569' }}>{positiveCount}</span></span>
            <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-red-500" /><span style={{ color: '#475569' }}>{negativeCount}</span></span>
          </div>
        </div>
        <div className="h-1 rounded-full overflow-hidden" style={{ background: 'rgba(99,102,241,0.06)' }}>
          <motion.div initial={{ width: 0 }} animate={{ width: `${positivePct}%` }}
            className="h-full rounded-full" style={{ background: `linear-gradient(90deg, #818cf8, rgba(99,102,241,0.4))` }} />
        </div>
      </div>
      ...
      {/* Asset List */}
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-0.5 relative z-10" style={{ scrollbarWidth: "thin", scrollbarColor: 'rgba(99,102,241,0.15) transparent' }}>
        {symbolsLoading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            <div className="relative w-12 h-12">
              <motion.div className="absolute inset-0 rounded-full"
                style={{ border: '2px solid rgba(99,102,241,0.1)', borderTopColor: '#818cf8' }}
                animate={{ rotate: 360 }}
                transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }} />
            </div>
            <p className="text-[10px] font-bold tracking-wider" style={{ color: '#475569' }}>{t("loadingSymbols")}</p>
          </div>
        ) : (
          <AnimatePresence mode="popLayout">
            {filtered.length > 0 ? filtered.map((asset) => {
              ...
              return (
                <motion.button key={asset.id} onClick={() => onSelectAsset(asset)}
                  ...
                  style={{
                    ...
                    border: selected ? '1px solid rgba(99,102,241,0.12)' : '1px solid transparent',
                    boxShadow: selected ? '0 2px 12px rgba(99,102,241,0.06)' : 'none',
                  }}>
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    <span className="text-[14px] flex-shrink-0 w-5 text-center">{icon}</span>
                    <div className="min-w-0">
                      <div className="text-[11px] font-black truncate text-white tracking-wider">
                        {asset.symbol.replace(".p", "")}
                      </div>
                      <div className="text-[8px] font-bold" style={{ color: '#475569' }}>{isRTL ? asset.name : asset.nameEn}</div>
                    </div>
                  </div>
                  <div
                  ...
                  <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-lg text-[10px] font-black"
                    style={{ background: pos ? "rgba(34,197,94,0.06)" : "rgba(239,68,68,0.06)", color: pos ? "#22c55e" : "#ef4444", border: `1px solid ${pos ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)'}` }}>
              ...
              <div className="text-center py-12">
                <Search className="w-8 h-8 mx-auto mb-2" style={{ color: '#475569' }} />
                <p className="text-[10px] font-bold" style={{ color: '#475569' }}>{t("noResults")}</p>
              </div>
            )}
          </AnimatePresence>
        )}
